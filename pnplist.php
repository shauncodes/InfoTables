...
<?php
$sql = "SELECT pid, pname, pproducts, paddy, pnumber, phours, ppin, pnotes FROM pnp";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "
					<tr>
							<td><a href=pnp.php?pop=".$row["pid"].">" . $row["pname"] . "</a></td>
							<td><a href=pnp.php?pop=".$row["pid"].">" . $row["paddy"] . "</a></td>
							<td><a href=tel:".$row["pnumber"].">" . $row["pnumber"] . "</a></td>
							<td><a href=pnp.php?pop=".$row["pid"].">" . $row["phours"] . "</a></td>
							<td><a target=new href=https://goo.gl/maps/".$row["ppin"].">" . $row["ppin"] . "</a></td>
					</tr>";
    }
} else {
	echo "<tr>
