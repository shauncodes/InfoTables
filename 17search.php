...
if(isset($_POST["search"])) { // Check to see if a search has been made.
	if(preg_match("/[A-Z  | a-z]+/", $_POST["search"])) { // Make sure the search starts with a letter
		...
		$sql = "SELECT * FROM 17joblist WHERE jobname LIKE '%{$search}%' OR jobgen LIKE '%{$search}%' or jobest LIKE '%{$search}%' or jobaddy LIKE '%{$search}%'";
		$result = $conn->query($sql);
		...
		if ($result->num_rows > 0) {
			...
			while($row = $result->fetch_assoc()) {
				echo "<tr>
						<td><a href=17jobs.php?job=".$row["jobnum"].">" . $row["jobnum"] . "</td>
						<td><a href=17jobs.php?job=".$row["jobnum"].">" . $row["jobname"] . "</td>
						<td><a href=17jobs.php?job=".$row["jobnum"].">" . $row["jobaddy"] . "</td>
