	...
	<div id="centerthis92">
		...
		<form action="pnp.php" method="get" style="display:inline;">
			<select name="pop" style="padding:3px 10px;" onchange="popSelect(this.value);">
				<option selected>Choose Pit/Plant</option>
				<?php
...
if(isset($_POST["search"])) { // Check to see if a search has been made.
	if(preg_match("/[A-Z  | a-z]+/", $_POST["search"])) { // Make sure the search starts with a letter
		...
		$pnpsearchsql = "SELECT * FROM pnp WHERE pname LIKE '%{$search}%' OR pproducts LIKE '%{$search}%' or paddy LIKE '%{$search}%' or psearch LIKE '%{$search}%'";
		$pnpresult = $conn->query($pnpsearchsql);
		if ($pnpresult->num_rows > 0) {
			// output data of each row
			while($pnprow = $pnpresult->fetch_assoc()) {
				echo "<tr>
						<td><a href=pnp.php?pop=".$pnprow["pid"].">" . $pnprow["pname"] . "</td>
						<td><a href=pnp.php?pop=".$pnprow["pid"].">" . $pnprow["paddy"] . "</td>
						<td><a href=tel:".$pnprow["pnumber"].">" . $pnprow["pnumber"] . "</td>
						<td><a href=pnp.php?pop=".$pnprow["pid"].">" . $pnprow["phours"] . "</td>
						<td>" . $pnprow["ppin"] . "</td>
				</tr>";
