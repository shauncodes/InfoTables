...
<script>
	function jobSelect(theval) {
		window.location.href = "19jobs.php?job=" + theval;
	}
</script>
...
$sql = "SELECT jobnum, jobgen, jobname, jobaddy, jobpin FROM 19joblist";
...
<nav class="navbar navbar-default">
	<div class="container-fluid">
		...
		<ul class="nav navbar-nav">
			...
			<li class="active"><a href="19joblist.php">2019 Joblist</a></li>
...
<div style="width:100%">
	<div id="centerthis92">
		...
		<div id="selectbar">
		<form action="19jobs.php" method="get" style="display:inline;">
			<select name="job" style="padding:3px 10px;" onchange="jobSelect(this.value);"> <!-- OnChange, run the function above with the selected jobnum -->
				<option selected>Job Number</option>
				<?php
				if ($result->num_rows > 0) { 							// This $result comes from the only query on the page,
					while($row = $result->fetch_assoc()) {				// which is just getting the whole DB, basically.
		...
		&nbsp;&nbsp;&nbsp;
		<form style="display:inline" action="19search.php" method="post">
...
<?php
$sql = "SELECT jobnum, jobname, jobaddy, jobpin FROM 19joblist";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        echo "
					<tr>
							<td><a href=19jobs.php?job=".$row["jobnum"].">" . $row["jobnum"] . "</td>
							<td><a href=19jobs.php?job=".$row["jobnum"].">" . $row["jobname"] . "</td>
							<td><a href=19jobs.php?job=".$row["jobnum"].">" . $row["jobaddy"] . "</td>
